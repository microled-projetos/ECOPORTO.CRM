...
namespace WsSimuladorCalculoTabelas.DAO
{
    ...
    public bool atualizaGREmCNTR(int lote, int seqGR)
                ...
                int idNotaSub)
        {
            ...
            try
            {
                using (OracleConnection con = new OracleConnection(Configuracoes.StringConexao()))
                {
                    ...
                    {
                        tipoPessoa = 1;
                        cgccli = Formata_CGCCPF(tipoDoc, sapCli.CGCCPF);
                    }
                    else
                    {
                        tipoPessoa = 2;
        ...
        public double Valor_Nota(string doc, string servico)
        {
            double Valor = 0;
            try
            {
                using (OracleConnection con = new OracleConnection(Configuracoes.StringConexao()))
                {
                    ...
                    sb.AppendLine("       FROM SGIPA.TB_SERVICOS_FATURADOS_IMPOSTOS GROUP BY AUTONUM_SERVICO_FATURADO ) B  ");
                    sb.AppendLine("   ON A.AUTONUM = B.AUTONUM_SERVICO_FATURADO  ");
                    sb.AppendLine(" WHERE A.SEQ_GR in(" + doc + ")  ");
        ...
        public double Valor_Desconto(string doc, string servico)
        {
            double valor = 0;
            try
            {
                using (OracleConnection con = new OracleConnection(Configuracoes.StringConexao()))
                {
                    ...
                    sb.AppendLine(" inner  Join  sgipa.tb_cad_impostos c on 1=1  where SEQ_GR IN(" + doc + ") And c.iss=1 GROUP BY SEQ_GR ) B ");
                    sb.AppendLine(" On A.SEQ_GR=B.SEQ_GR ");
                    sb.AppendLine(" WHERE A.SEQ_GR IN(" + doc + ") ");
                    sb.AppendLine(" AND A.AUTONUM IN(" + servico + ") ");
